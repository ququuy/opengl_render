...
// --------------------------------------------------------------------------------
// Constructor
FreeLookCamera::FreeLookCamera(GLFWwindow *window, glm::vec3 position, glm::quat orientation) : Camera(window) {
	// Orientation
	mOrientation = orientation;//glm::quat(1, 0, 0, 0);

	// Position
	mPosition = position;//glm::vec3(0, 0, 5);
...
// Camera Reflection Matrix
glm::mat4 FreeLookCamera::getReflectedViewMatrix(glm::vec3 position, glm::vec3 normal) {
	// Generate Forward, Right, Up Vectors
	glm::vec3 forward = glm::vec3(mOrientation * glm::vec4( 0,  0, -1,  0));
	glm::vec3 up      = glm::vec3(mOrientation * glm::vec4( 0,  1,  0,  0));

	glm::vec3 reflected_position = mPosition - 2.0f * glm::dot(glm::vec3(mPosition) - position, normal) * normal;
	glm::vec3 reflected_forward = forward - 2.0f * glm::dot(forward, normal) * normal;
	glm::vec3 reflected_up = up - 2.0f * glm::dot(up, normal) * normal;

	return glm::lookAt(reflected_position, reflected_position + reflected_forward, reflected_up);
}

glm::vec4 FreeLookCamera::getPosition() {
    return glm::vec4 (mPosition, 1.0);
}
...
// --------------------------------------------------------------------------------
// Constructor
GimbalFreeLookCamera::GimbalFreeLookCamera(GLFWwindow *window, glm::vec3 position, glm::quat orientation) : FreeLookCamera(window, position, orientation) {
	// View Angles
	mViewAngleX = 0.0f;
	mViewAngleY = 0.0f;
}
