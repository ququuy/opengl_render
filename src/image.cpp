...
// Load an image from file
float* loadHDRImage(const char *filename, int &width, int &height, int &n, bool flip) {
	// Force RGBA 
	int force_channels = n;

	// Load image
	float *image = stbi_loadf(filename, &width, &height, &n, force_channels);
	...
	if(flip) {
		...
		for(int iy = 0; iy < height/2; iy++) {
			// Copy row iy into temporary buffer
			memcpy(t, &image[iy*width*n], width*n*sizeof(float));

			// Copy row ((height-1)-iy) into row iy
			memcpy(&image[iy*width*n], &image[((height-1)-iy)*width*n], width*n*sizeof(float));

			// Copy temporary buffer into row ((height-1)-iy)
			memcpy(&image[((height-1)-iy)*width*n], t, width*n*sizeof(float));
		}
	}
...
// Load an HDR Texture from file
GLuint loadHDRTexture2D(const char *filename, int &width, int &height, int &n, GLint minFilter, GLint magFilter, GLint textureWrapS, GLint textureWrapT) {
	// ----------------------------------------
	// Load Texture Map from file
	float *image = loadHDRImage(filename, width, height, n, false);
	...
	// ------------------------------
	// Set storage - log_2(image size)
	glTexStorage2D(GL_TEXTURE_2D, max_levels, GL_RGBA16F, width, height);

	// Copy image data into texture
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_FLOAT, image);
